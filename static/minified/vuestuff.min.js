...
let vuePlayerReport=new Vue({el:"#div-modal",data:{selectedPlayer:null,kills:null,damageDealt:null,DBNOs:null,timeSurvived:null,winPlace:null,teamKills:null,rowId:null,arrPlayerReport:[],isHidden:!0,hitLocations:{},},methods:{updatePlayerReport:function(name,killer,playerTeam,killerTeam,arrPlayerCards,arrPlayersDamageLog,allBotNames,allHumanNames){if(allBotNames.includes(name)){document.getElementById('botReportDisclaimer').style.display='block';document.getElementById('div-reportStats').style.display='none'}else{document.getElementById('botReportDisclaimer').style.display='none';document.getElementById('div-reportStats').style.display='block'}
this.selectedPlayer=name;this.hitLocations.head=0;this.hitLocations.body=0;this.hitLocations.pelvis=0;this.hitLocations.arm=0;this.hitLocations.leg=0;this.kills='(invalid, bot)';this.damageDealt='(invalid, bot)';this.DBNOs='(invalid, bot)';this.timeSurvived='(invalid, bot)';this.winPlace='(invalid, bot)';this.teamKills='(invalid, bot)';arrPlayerCards.forEach(element=>{if(element.name==name){this.kills=element.kills;this.damageDealt=parseInt(element.damageDealt).toLocaleString('en');this.DBNOs=element.DBNOs;this.timeSurvived=element.timeSurvived;this.winPlace=element.winPlace;this.teamKills=element.teamKills}});rowId=0;this.arrPlayerReport=[];arrPlayersDamageLog.forEach(record=>{if(record.attacker.name==this.selectedPlayer||record.victim.name==this.selectedPlayer){let _event='';let _info='';let _distance='';let attackerName=record.attacker.name;let victimName=record.victim.name;let attackerHealth='';let victimHealth='';let armor='';let attackerClass='';let victimClass='';let rowClass='';let zone='';if(record._T=='LogPlayerTakeDamage'){if(attackerName==name){if(record.damageReason=='HeadShot'){this.hitLocations.head++}else if(record.damageReason=='TorsoShot'){this.hitLocations.body++}else if(record.damageReason=='PelvisShot'){this.hitLocations.pelvis++}else if(record.damageReason=='LegShot'){this.hitLocations.leg++}else if(record.damageReason=='ArmShot'){this.hitLocations.arm++}}
...
if(victimName==killer&&victimName!=name){victimClass='killer';if(allBotNames.includes(killer)){victimName='[BOT] '+victimName}}
if(attackerName!=killer&&killer!=''&&killerTeam!=0){let killedByTeammate=0;killerTeam.teammates.forEach(teammate=>{if(teammate.name==killer){killedByTeammate++}else if(teammate.name==name){killedByTeammate++}});killerTeam.teammates.forEach(teammate=>{if(attackerName==teammate.name){if(teammate.isBot){attackerName='[BOT] '+attackerName}
if(killedByTeammate!=2){attackerClass='killerTeammate'}}})}
if(victimName!=killer&&killer!=''&&killerTeam!=0){let killedByTeammate=0;killerTeam.teammates.forEach(teammate=>{if(teammate.name==killer){killedByTeammate++}else if(teammate.name==name){killedByTeammate++}});killerTeam.teammates.forEach(teammate=>{if(victimName==teammate.name){if(teammate.isBot){victimName='[BOT] '+victimName}
if(killedByTeammate!=2){victimClass='killerTeammate'}}})}
...
return _damager},resolveDamageReason:function(damageCauserName,damageReason,damageTypeCategory){let r='';if(damageReason!=undefined&&damageReason!='NonSpecific'){r=damageReason;if(damageReason=='TorsoShot'){r='Body'}else if(damageReason=='LegShot'){r='Leg'}else if(damageReason=='ArmShot'){r='Arm'}else if(damageReason=='HeadShot'){r='Headshot'}else if(damageReason=='PelvisShot'){r='Pelvis'}}
